...
static void APP_Dispatch(uint16_t cmd, uint8_t *data, uint16_t len)
{
    ...
    case MGR_CMD_QUERY_ADC: {
        ...
        if (len >= 2 && data[0] == ADC_CONSISTENCY_TOKEN_ALL) {
            /* [2026-08-20] U4 离线快速失败: 一致性校验的 U4 数据来自报文流,
             * 离线时用脏数据比对无意义, 且会让上位机空等 → 立即回超时错误 */
            if (!U4_IsOnline()) {
                uint8_t e = APP_STATUS_ERR_TIMEOUT; APP_SendAck(cmd, &e, 1); break;
            }
    ...
    case MGR_CMD_ADC_CALIB: {
        /* [2026-08-20] U4 离线快速失败: 极值数据来自 U4 报文流, 离线空跑 24 段电机
         * 流程 (~16s) 无意义且上位机易超时 → 立即回超时错误 */
        if (!U4_IsOnline()) {
            uint8_t e = APP_STATUS_ERR_TIMEOUT; APP_SendAck(cmd, &e, 1); break;
        }
    ...
    case MGR_CMD_DIAM_CALIB: {
        /* [2026-08-20] U4 离线快速失败: DUT 即 U4, 离线时 U4_ZeroSensor 必失败,
         * 提前返回明确错误避免上位机等待 */
        if (!U4_IsOnline()) {
            uint8_t e = APP_STATUS_ERR_TIMEOUT; APP_SendAck(cmd, &e, 1); break;
        }
